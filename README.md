## What is PackMe
Two words: binary serialization. Or rather, four words: Blazing Fast Binary Serialization (BFBS). PackMe is a lightweight library that allows using your JSON description of data protocols to generate all necessary classes (JS, Flutter/Dart) for your client-server binary data communication.
<table><tr><td>
...
```javascript
// ... Simple as that!
let request = new QuesitonRequest(
    "What is the Matrix?",
    new Date()
);
let data = await serverQuery(packMe.pack(request));

let response = packMe.unpack(data);
response.answers.forEach(console.log);
```

</td></tr></table>

## Performance
BFBS. Spoiler alert! 500,000+ pack/unpack cycles per second for data of average size and complexity (see example.js).

Since PackMe generates .js classes, there is no need for resource demanding serialization/deserialization process. With no intermediate steps involved, each class has efficient methods to quickly place all data into a Uint8Array buffer and extract it. Compared to popular solutions, it completely outmatches JSON or Protobuf in terms of performance.
...
## Usage
The most effective way to use it in client-server applications is through the [ConnectMe](https://github.com/sourcecaster/connectme.js) package, which provides essential features like message listeners and asynchronous queries. Nevertheless, it can also be used separately.
...
npx packme packme generated
```
Next, let's import manifest.generated.js and proceed with a simple code for both client and server sides. Here goes the client side first:
```javascript
...
"some_hash": "binary64"
```
The declaration above defines two Uint8Array fields: one with a length of 12 bytes and another with a length of 64 bytes.
