...
function parse(nodes) {
    ...
    for (let filename of Object.keys(codePerFile)) {
        let ownEnums = Object.values(enums).filter((item) => item.filename === filename);
        let ownTypes = Object.values(types).filter((item) => item.filename === filename);
        let ownMessages = [...messages.entries()].filter((entry) => entry[1].filename === filename);
        codePerFile[filename] = [
            "import {PackMe, PackMeMessage} from 'packme';",
            '',
            ...ownEnums.reduce((a, b) => a.concat(b.output()), []),
            ...ownTypes.reduce((a, b) => a.concat(b.output()), []),
            ...ownMessages.reduce((a, b) => a.concat(b[1].output()), []),
            ...(ownMessages.length > 0 ? [
                `const ${validName(filename)}MessageFactory = {`,
                ...ownMessages.map((entry) => `'${entry[0]}': () => new ${entry[1].name}(),`),
                '};'
            ] : [])
        ];
        ...
        let exports = [
            ...ownEnums.map((item) => item.name),
            ...ownTypes.map((item) => item.name),
            ...ownMessages.map((entry) => entry[1].name),
            ...(ownMessages.length > 0 ? [`${validName(filename)}MessageFactory`] : [])
        ];
